...
function displayResults() {
  ...
  if (loadAllResult) {
    ...
    document.getElementById("docsLoadAll").textContent =
      loadAllResult.docsLoaded;
    document.getElementById("tokensLoadAll").textContent =
      loadAllResult.usage.input_tokens.toLocaleString();
    document.getElementById("costLoadAll").textContent =
      "$" + loadAllResult.cost.toFixed(4);
    document.getElementById("timeLoadAll").textContent =
      (loadAllResult.responseTime / 1000).toFixed(2) + "s";
  }
  ...
  if (smartResult) {
    ...
    document.getElementById("tokensSmart").textContent =
      smartResult.usage.input_tokens.toLocaleString();
    document.getElementById("costSmart").textContent =
      "$" + smartResult.cost.toFixed(4);
    document.getElementById("timeSmart").textContent =
      (smartResult.responseTime / 1000).toFixed(2) + "s";
    ...
    if (
      smartResult.cacheStats.cacheCreationTokens > 0 ||
      smartResult.cacheStats.cacheReadTokens > 0
    ) {
    ...
    if (smartResult.selectedDocs && smartResult.selectedDocs.length > 0) {
      ...
      selectedDocsList.textContent = smartResult.selectedDocs
        .map((d) => d.filename)
        .join(", ");
    }
  }
...
function showComparison() {
  ...
  const tokenReduction = (
    (1 - smartResult.usage.input_tokens / loadAllResult.usage.input_tokens) *
    100
  ).toFixed(1);
  const costSavings = (
    (1 - smartResult.cost / loadAllResult.cost) *
    100
  ).toFixed(1);
  const speedImprovement = (
    loadAllResult.responseTime / smartResult.responseTime
  ).toFixed(1);
...
async function resetSystem() {
  if (
    !confirm("This will delete all documents and reset the budget. Continue?")
  ) {
    return;
  }
